...
func TestUpdateAccount(t *testing.T) {
	account1 := createRandomAccount(t)
	cmd := UpdateAccountParams{
		ID: account1.ID,
		Balance: util.RandomMoney(0.0, 999999.99),
	}
	
	account2, err := testQueries.UpdateAccount(context.Background(), cmd)
	require.NoError(t, err)
	require.NotEmpty(t, account2)

	require.Equal(t, account1.ID, account2.ID)
	require.Equal(t, account2.Balance, cmd.Balance)
	require.Equal(t, account1.Owner, account2.Owner)
	require.Equal(t, account1.Currency, account2.Currency)
	require.WithinDuration(t, account1.CreatedAt, account2.CreatedAt, time.Second)
}
...
func TestListAccount(t *testing.T) {
	...
	cmd := ListAccountsParams{
		Limit: 5,
		Offset: 5,
	}

	accounts, err := testQueries.ListAccounts(context.Background(), cmd)
	require.NoError(t, err)
